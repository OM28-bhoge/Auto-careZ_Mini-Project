...
def feedback():
    ...
    if request.method == 'POST':
        ...
        cursor.execute("insert into feedback (fullname,email_id,phoneno,message) values (%s,%s,%s,%s)",
                       (fullname, email_id, phoneno, message,))
...
def usersignup():
    ...
    if request.method == 'POST' and 'fullname' in request.form and 'phoneno' in request.form and 'email_id' in request.form and 'password' in request.form:
        ...
        if output:
            ...
            cursor.execute("insert into project.customer (fullname,phoneno,email_id,password) values (%s,%s,%s,%s)",
                           (fullname, phoneno, email_id, password,))
            mysql.connection.commit()
...
def mechanicbillpending():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select billno,name,model from project.adminserviceform,project.Bill where adminrespond=%s and billstatus=%s and serviceby=%s and adminserviceform.s_id=Bill.s_id",
        ("completed", "notpaid", session['mechanic'],))
    data = cursor.fetchall()
    print(data)
    cursor.close()
...
def forgotpassword():
    ...
    if request.method == 'POST' and 'fullname' in request.form and 'email_id' in request.form and 'password' in request.form:
        ...
        acc = cursor.fetchone()
        if acc:
            cursor.execute('update customer set password = %s where fullname = %s and email_id = %s',
                           (password, fullname, email_id))
            mysql.connection.commit()
            cursor.close()
...
def graph():
    ...
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select service_type,count(*) from adminserviceform where month(r_date)=month(current_date()) group by service_type")
...
def admindashboard():
    ...
    for value in data4:
        c5 = c5 + 1
    cursor.execute(
        "select* from project.adminserviceform,Bill where adminrespond=%s and billstatus=%s and adminserviceform.s_id=Bill.s_id",
        ("completed", "paid",))
    data5 = cursor.fetchall()
    c6 = 0
    for value in data5:
...
def newservice():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select service_id,s_id,name,r_date,email_id from project.serviceform,project.customer where serviceform.customer_id=customer.customer_id")
    data = cursor.fetchall()
    print(data)
...
def editnewservice(service_id):
    ...
    print(data1)
    print(data2)
    return render_template('editnewservice.html', customer=data, customer1=data1,data2=data2,service=service)
...
def serviceformrespond(service_id):
    if request.method == 'POST':
        ...
        print(adminrespond)
        cursor.execute(
            "select name,model,cartype,r_no,service_type,date,time,deliverytype,pickup,dropaddress,r_date,customer_id,pincode from serviceform where s_id=%s",
            (service_id,))
        data = cursor.fetchall()
        print(data)
        ...
        for value in data:
            ...
            if session['delivery'] == "pickdrop":
                cursor.execute(
                    "insert into adminserviceform (s_id,name,model,cartype,r_no,service_type,pickupdate,pickuptime,dropdate,droptime,deliverytype,pickup,dropaddress,r_date,customer_id,adminrespond,pincode) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                    (service_id, value[0], value[1], value[2], value[3], value[4], value[5], value[6], dropdate,
                     droptime, value[7], value[8], value[9], value[10], value[11], adminrespond, value[12],))
                mysql.connection.commit()
                ...
                cursor.execute(
                    "insert into adminserviceform (s_id,name,model,cartype,r_no,service_type,pickupdate,pickuptime,deliverytype,pickup,dropaddress,r_date,customer_id,adminrespond,pincode) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                    (service_id, value[0], value[1], value[2], value[3], value[4], value[5], value[6], value[7],
                     value[8], value[9], value[10], value[11], adminrespond, value[12],))
                mysql.connection.commit()
                cursor.execute("delete from serviceform where s_id=%s", (service_id,))
                mysql.connection.commit()
                ...
                cursor.execute(
                    "insert into adminserviceform (s_id,name,model,cartype,r_no,service_type,r_date,customer_id,adminrespond,pincode) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                    (service_id, value[0], value[1], value[2], value[3], value[4], value[10], value[11], adminrespond,
                     value[12],))
                mysql.connection.commit()
                cursor.execute("delete from serviceform where s_id=%s", (service_id,))
                mysql.connection.commit()
        ...
        if adminrespond == "accept":
            ...
            cursor.execute('insert into serviceformreject (s_id,description) values (%s,%s)',
...
def serviceformpending():
    cursor = mysql.connection.cursor()
    cursor.execute("select service_id,s_id,name,r_date from project.adminserviceform where adminrespond=%s",
                   ("pending",))
    data = cursor.fetchall()
    cursor.close()
...
def pendingupdate(service_id):
    if request.method == 'POST':
        ...
        data5 = cursor.fetchone()
        print(data5)
        cursor.execute("update adminserviceform set adminrespond=%s, serviceby=%s where s_id=%s",
                       (adminrespond, serviceby, service_id,))
        mysql.connection.commit()
        word = ""
        for value in data5:
            word = value
        ...
        cursor.execute(
            "select s_id,name,model,cartype,r_no,service_type,deliverytype,pickup,pickupdate,pickuptime,dropaddress,dropdate,droptime,r_date,responddate,adminrespond,pincode from adminserviceform where s_id=%s",
            (service_id,))
        data = cursor.fetchall()
        word = ""
        car = ""
...
def billpending():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select billno,name,model from project.adminserviceform,project.Bill where adminrespond=%s and billstatus=%s and adminserviceform.s_id=Bill.s_id",
        ("completed", "notpaid",))
    data = cursor.fetchall()
    print(data)
    cursor.close()
...
def servicecompleted():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select billno,name,model from project.adminserviceform,project.Bill where adminrespond=%s and billstatus=%s and adminserviceform.s_id=Bill.s_id order by r_date",
        ("completed", "paid",))
    data = cursor.fetchall()
    print(data)
    cursor.close()
...
def servicecompletedview(billno):
    ...
    data3 = cursor.fetchall()
    cursor.execute(
        "select name,s_id,model,cartype,r_no,adminrespond,serviceby from adminserviceform where s_id=%s order by r_date",
        (data3,))
    data1 = cursor.fetchall()
    for value in data1:
        print(value[1])
...
def totalservicediscount(d1, d2):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select s_id,name,model,cartype,r_no,service_type,deliverytype,pickup,pickupdate,pickuptime,dropaddress,dropdate,droptime,r_date,responddate,adminrespond,pincode from adminserviceform where service_id=%s and s_id=%s",
        (d1, d2,))
    data = cursor.fetchall()
    word = ""
    car = ""
...
def adddiscount(s_id):
    ...
    if request.method == 'POST':
        ...
        cursor.execute(
            "insert into Bill (billno,totalprice,discount,netprice,GST,totalbill,customer_id,s_id,billstatus) values (%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            (billno, totalprice, discount, netbill, GST, total, data, s_id, "notpaid",))
        mysql.connection.commit()
        cursor.close()
...
def viewbill():
    ...
    data3 = cursor.fetchall()
    cursor.execute("select totalprice,discount,netprice,GST,totalbill from Bill where s_id=%s and customer_id=%s",
                   (session['s_id'], data,))
    data4 = cursor.fetchall()
    cursor.close()
...
def serviceformreject():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select service_id,s_id,model,r_date,responddate from project.adminserviceform where adminrespond=%s order by r_date",
        ("reject",))
    data = cursor.fetchall()
...
def rejectedservicereportview():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,model,service_type,responddate,description from adminserviceform,serviceformreject where adminrespond=%s and adminserviceform.s_id=serviceformreject.s_id and r_date between %s and %s order by r_date",
        ("reject", session['date1'], session['date2'],))
    data = cursor.fetchall()
...
def rejectedservicereport():
    ...
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Rejected_Service_Report.pdf'})
...
def pendingbillreportview():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,model,cartype,service_type,responddate,totalbill from adminserviceform,Bill where adminrespond=%s and adminserviceform.s_id=Bill.s_id and billstatus=%s and r_date between %s and %s order by r_date",
        ("completed", "notpaid", session['date1'], session['date2'],))
    data = cursor.fetchall()
...
def pendingbillreport():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Pending_Bill_Report.pdf'})
...
def repeatedcustomerview():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "SELECT max(Total) FROM (SELECT COUNT(*) AS Total FROM adminserviceform where adminrespond=%s GROUP BY name,model,cartype) AS Results",
        ("completed",))
    m = cursor.fetchone()
    # print(m[0])
    cursor.execute(
        'select name,model,cartype,count(*) from adminserviceform where adminrespond=%s group by name,model,cartype',
        ('completed',))
    data = cursor.fetchall()
    # print(data)
    i = 0
    ...
    for value in data:
        ...
        if i == m[0]:
            ...
            print('hi')
            cursor.execute(
                "select name,model,cartype,service_type,responddate,serviceby,totalbill from adminserviceform,Bill where name=%s and model=%s and cartype=%s and adminrespond=%s and adminserviceform.s_id=Bill.s_id order by r_date",
                (v1, v2, v3, "completed",))
            data1 = cursor.fetchall()
            new.append(data1)
...
def repeatedcustomer():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Most_repeated_Customer_Report.pdf'})
...
def pendingservicereportview():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,model,cartype,service_type,r_date,r_no from adminserviceform where adminrespond=%s and r_date between %s and %s order by r_date",
        ("pending", session['date1'], session['date2'],))
    data = cursor.fetchall()
...
def pendingservicereport():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Pending_Service_Report.pdf'})
...
def customerreport():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Total_Customer_Report.pdf'})
...
def updatedrepairreport():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select carname,servicename,price,updateprice,description,date from project.repairupdate where date between %s and %s order by date",
...
def repairupdateddownload():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Updated_Repair_Service_Report.pdf'})
...
def deletedrepairreport():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select carname,servicename,price,description,date from project.deleterepair where date between %s and %s order by date",
        (session['date1'], session['date2'],))
    data = cursor.fetchall()
...
def repairdeleteddownload():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Deleted_Repair_Service_Report.pdf'})
...
def updatedtyrereport():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select carname,name,size,price,updateprice,description,date from project.tyreupdate where date between %s and %s order by date",
        (session['date1'], session['date2'],))
    data = cursor.fetchall()
...
def tyreupdateddownload():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Updated_Tyre_Service_Report.pdf'})
...
def updatedbatteryreport():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select carname,batteryname,capacity,price,updateprice,description,date from project.batteryupdate where date between %s and %s order by date",
        (session['date1'], session['date2'],))
    data = cursor.fetchall()
...
def batteryupdateddownload():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Updated_Battery_Service_Report.pdf'})
...
def deletedtyrereport():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select carname,name,size,price,description,date from project.deletetyre where date between %s and %s order by date",
        (session['date1'], session['date2'],))
    data = cursor.fetchall()
...
def tyredeleteddownload():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Deleted_Tyre_Service_Report.pdf'})
...
def deletedbatteryreport():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select carname,batteryname,capacity,price,description,date from project.deletebattery where date between %s and %s order by date",
        (session['date1'], session['date2'],))
    data = cursor.fetchall()
...
def batterydeleteddownload():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Deleted_Battery_Service_Report.pdf'})
...
def areareportview():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,model,cartype,service_type,responddate,totalbill,pincode from adminserviceform,Bill where adminrespond=%s and adminserviceform.s_id=Bill.s_id and billstatus=%s and r_date between %s and %s",
        ("completed", "paid", session['date1'], session['date2'],))
    data1 = cursor.fetchall()
    cursor.execute(
        "select name,model,cartype,service_type,responddate,totalbill,pincode,count(*) from adminserviceform,Bill where adminrespond=%s and adminserviceform.s_id=Bill.s_id and billstatus=%s and r_date between %s and %s group by pincode",
        ("completed", "paid", session['date1'], session['date2'],))
    data = cursor.fetchall()
    x = []
    y = []
...
def areareportdownload():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,model,cartype,service_type,responddate,totalbill,pincode from adminserviceform,Bill where adminrespond=%s and adminserviceform.s_id=Bill.s_id and billstatus=%s and r_date between %s and %s order by r_date",
        ("completed", "paid", session['date1'], session['date2'],))
    data1 = cursor.fetchall()
    cursor.execute(
        "select name,model,cartype,service_type,responddate,totalbill,pincode,count(*) from adminserviceform,Bill where adminrespond=%s and adminserviceform.s_id=Bill.s_id and billstatus=%s and r_date between %s and %s group by pincode",
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Pincode_Service_Report.pdf'})
...
def highservice():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select service_type,count(*),model,cartype from adminserviceform where  r_date between %s and %s GROUP BY service_type",
        (session['date1'], session['date2'],))
    data5 = cursor.fetchall()
    cursor.execute("select servicename from service")
    data6 = cursor.fetchall()
...
def highservicereportdownload():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select model,cartype,service_type,count(*) from adminserviceform where  r_date between %s and %s GROUP BY service_type",
        (session['date1'], session['date2'],))
    data5 = cursor.fetchall()
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=High_Service_Report.pdf'})
...
def completedreportview():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,model,cartype,service_type,responddate,totalbill,billdate from adminserviceform,Bill where adminrespond=%s and adminserviceform.s_id=Bill.s_id and billstatus=%s and r_date between %s and %s order by r_date",
        ("completed", "paid", session['date1'], session['date2'],))
    data = cursor.fetchall()
...
def completedreportdownload():
    ...
    pdf.cell(page_width, 0.0, '-----End of report-----', align='C')
    return Response(pdf.output(dest='S').encode('latin-1'), mimetype="application/pdf",
                    headers={'Content-Disposition': 'attachment;filename=Completed_Service_Report.pdf'})
...
def userserviceform():
    ...
    data = cursor.fetchone()
    print(data)
    cursor.execute("select service_id,s_id,r_date,responddate from project.adminserviceform where customer_id=%s",
                   (data,))
    data = cursor.fetchall()
    print(data)
...
def userserviceformview(service_id):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select s_id,name,model,cartype,r_no,service_type,deliverytype,pickup,pickupdate,pickuptime,dropaddress,dropdate,droptime,adminrespond,serviceby,pincode from adminserviceform,customer where adminserviceform.customer_id=customer.customer_id and service_id=%s",
        (service_id,))
    data = cursor.fetchall()
    cursor.execute(
        "select servicename from service,adminserviceform where adminserviceform.s_id=service.s_id and service_id=%s",
        (service_id,))
    data1 = cursor.fetchall()
    for value in data:
        print(value[13])
...
def userservceformbillview(s_id):
    ...
    data4 = cursor.fetchall()
    cursor.close()
    return render_template('usertotalbill.html', data1=data1, data2=data2, data3=data3, data4=data4,
                           serviceno=serviceno)
    ...
    if 'loggedin' in session:
        ...
        if request.method == 'POST' and 'fullname' in request.form and 'phoneno' in request.form and 'email_id' in request.form:
            ...
            if not re.match(r'[^@]+@[^@]+\.[^@]+', email_id):
                ...
                cursor.execute(
                    'UPDATE customer SET  fullname =% s, phoneno =% s, email_id =% s  WHERE customer_id =% s',
                    (fullname, phoneno, email_id, (session['id'],),))
                mysql.connection.commit()
...
def repair():
    ...
    print(data10)
    return render_template('repair.html', service1=data1, service2=data2, service3=data3, service4=data4,
                           service5=data5, service6=data6, service7=data7, service8=data8, service9=data9,
                           service10=data10, service11=data11, service12=data12, service13=data13, service14=data14,
                           service15=data15)
...
def batterybalenodescription(batteryname):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select batteryname from batterytype,model where model.model_id=batterytype.model_id and carname=%s and batteryname=%s",
        (session[model], batteryname,))
    data = cursor.fetchone()
    cursor.close()
...
def batteryciazdescription(batteryname):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select batteryname from batterytype,model where model.model_id=batterytype.model_id and carname='ciaz' and batteryname=%s",
        (batteryname,))
    data = cursor.fetchone()
    print(data)
    cursor.close()
...
def batterydziredescription(batteryname):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select batteryname from batterytype,model where model.model_id=batterytype.model_id and carname='dzire' and batteryname=%s",
        (batteryname,))
    data = cursor.fetchone()
    cursor.close()
...
def batteryspressodescription(batteryname):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select batteryname from batterytype,model where model.model_id=batterytype.model_id and carname='spresso' and batteryname=%s",
        (batteryname,))
    data = cursor.fetchone()
    cursor.close()
    return render_template("batterybalenodescription.html", data=data)
...
def batteryswiftdescription(batteryname):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select batteryname from batterytype,model where model.model_id=batterytype.model_id and carname='swift' and batteryname=%s",
        (batteryname,))
    data = cursor.fetchone()
    cursor.close()
...
def repairbalenodescription(servicename):
    ...
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select servicename from repairtype,model where model.model_id=repairtype.model_id and carname='Baleno' and servicename=%s",
        (servicename,))
    data = cursor.fetchone()
    print(data)
    cursor.close()
...
def repairciazdescription(servicename):
    ...
    print(servicename)
    cursor.execute(
        "select servicename from repairtype,model where model.model_id=repairtype.model_id and carname='Ciaz' and servicename=%s",
        (servicename,))
    data = cursor.fetchone()
    cursor.close()
...
def repairdziredescription(servicename):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select servicename from repairtype,model where model.model_id=repairtype.model_id and carname='Dzire' and servicename=%s",
        (servicename,))
    data = cursor.fetchone()
    cursor.close()
...
def repairspressodescription(servicename):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select servicename from repairtype,model where model.model_id=repairtype.model_id and carname='Spresso' and servicename=%s",
        (servicename,))
    data = cursor.fetchone()
    cursor.close()
...
def repairswiftdescription(servicename):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select servicename from repairtype,model where model.model_id=repairtype.model_id and carname='Swift' and servicename=%s",
        (servicename,))
    data = cursor.fetchone()
    cursor.close()
...
def tyrebalenodescription(name, size):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,size from tyrename inner join tyresize on tyresize.t_id=tyrename.t_id inner join tyremodel on tyremodel.tsize_id=tyresize.tsize_id inner join model on model.model_id=tyremodel.model_id where carname='Baleno' and name=%s and size=%s",
        (name, size))
    data = cursor.fetchone()
    cursor.close()
...
def tyreciazdescription(name, size):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,size from tyrename inner join tyresize on tyresize.t_id=tyrename.t_id inner join tyremodel on tyremodel.tsize_id=tyresize.tsize_id inner join model on model.model_id=tyremodel.model_id where carname='Ciaz' and name=%s and size=%s",
        (name, size))
    data = cursor.fetchone()
    cursor.close()
...
def tyredziredescription(name, size):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,size from tyrename inner join tyresize on tyresize.t_id=tyrename.t_id inner join tyremodel on tyremodel.tsize_id=tyresize.tsize_id inner join model on model.model_id=tyremodel.model_id where carname='Spresso' and name=%s and size=%s",
        (name, size))
    data = cursor.fetchone()
    cursor.close()
...
def tyreswiftdescription(name, size):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select name,size from tyrename inner join tyresize on tyresize.t_id=tyrename.t_id inner join tyremodel on tyremodel.tsize_id=tyresize.tsize_id inner join model on model.model_id=tyremodel.model_id where carname='Swift' and name=%s and size=%s",
        (name, size))
    data = cursor.fetchone()
    cursor.close()
...
def repairbalenoupdate(service1):
    if request.method == 'POST':
        ...
        data = cursor.fetchone()
        cursor.execute(
            'insert into repairupdate (carname,servicename,price,updateprice,description) values (%s,%s,%s,%s,%s)',
            ('Baleno', service1, data, price, description))
        cursor.fetchone()
...
def repairciazupdate(service1):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into repairupdate (carname,servicename,price,updateprice,description) values (%s,%s,%s,%s,%s)',
            ('Ciaz', service1, data, price, description))
        cursor.fetchone()
...
def repairdzireupdate(service1):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into repairupdate (carname,servicename,price,updateprice,description) values (%s,%s,%s,%s,%s)',
            ('Dzire', service1, data, price, description))
        cursor.fetchone()
...
def repairspressoupdate(service1):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into repairupdate (carname,servicename,price,updateprice,description) values (%s,%s,%s,%s,%s)',
            ('Spresso', service1, data, price, description))
        cursor.fetchone()
...
def repairswiftupdate(service1):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into repairupdate (carname,servicename,price,updateprice,description) values (%s,%s,%s,%s,%s)',
            ('Swift', service1, data, price, description))
        cursor.fetchone()
...
def tyrebalenoupdate(name, size):
    if request.method == 'POST':
        ...
        description = request.form['description']
        cursor = mysql.connection.cursor()
        cursor.execute('select price from tyresize,tyrename where tyrename.t_id=tyresize.t_id and size=%s and name=%s',
                       (size, name))
        data = cursor.fetchone()
        cursor.execute(
            'insert into tyreupdate (carname,name,size,price,updateprice,description) values (%s,%s,%s,%s,%s,%s)',
            ('Baleno', name, size, data, price, description))
        cursor.fetchone()
...
def tyreswiftupdate(name, size):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into tyreupdate (carname,name,size,price,updateprice,description) values (%s,%s,%s,%s,%s,%s)',
            ('Swift', name, size, data, price, description))
        cursor.fetchone()
...
def tyrespressoupdate(name, size):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into tyreupdate (carname,name,size,price,updateprice,description) values (%s,%s,%s,%s,%s,%s)',
            ('Spresso', name, size, data, price, description))
        cursor.fetchone()
...
def tyredzireupdate(name, size):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into tyreupdate (carname,name,size,price,updateprice,description) values (%s,%s,%s,%s,%s,%s)',
            ('Dzire', name, size, data, price, description))
        cursor.fetchone()
...
def tyreciazupdate(name, size):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into tyreupdate (carname,name,size,price,updateprice,description) values (%s,%s,%s,%s,%s,%s)',
            ('Ciaz', name, size, data, price, description))
        cursor.fetchone()
...
def deletebalenotyre(name, size):
    ...
    if request.method == 'POST':
        ...
        cursor.execute('select price from tyresize,tyrename where tyrename.t_id=tyresize.t_id and size=%s and name=%s',
                       (size, name))
        data1 = cursor.fetchone()
        cursor.execute('insert into deletetyre (carname,name,size,price,description) values (%s,%s,%s,%s,%s)',
                       ('Baleno', name, size, data1, description))
...
def deleteswifttyre(name, size):
    ...
    if request.method == 'POST':
        ...
        data1 = cursor.fetchone()
        cursor.execute('insert into deletetyre (carname,name,size,price,description) values (%s,%s,%s,%s,%s)',
                       ('Swift', name, size, data1, description))
        cursor.execute('select tyresize.tsize_id,tyrename.t_id from tyresize,tyrename where size=%s and name=%s',
                       (size, name))
        data = cursor.fetchone()
        print(data)
...
def deletespressotyre(name, size):
    ...
    if request.method == 'POST':
        ...
        data1 = cursor.fetchone()
        cursor.execute('insert into deletetyre (carname,name,size,price,description) values (%s,%s,%s,%s,%s)',
                       ('Spresso', name, size, data1, description))
        cursor.execute('select tsize_id,tyrename.t_id from tyresize,tyrename where size=%s and name=%s', (size, name))
        data = cursor.fetchone()
        print(data)
...
def deletedziretyre(name, size):
    ...
    if request.method == 'POST':
        ...
        data1 = cursor.fetchone()
        cursor.execute('insert into deletetyre (carname,name,size,price,description) values (%s,%s,%s,%s,%s)',
                       ('Dzire', name, size, data1, description))
        cursor.execute('select tsize_id,tyrename.t_id from tyresize,tyrename where size=%s and name=%s', (size, name))
        data = cursor.fetchone()
        print(data)
...
def deleteciaztyre(name, size):
    ...
    if request.method == 'POST':
        ...
        data1 = cursor.fetchone()
        cursor.execute('insert into deletetyre (carname,name,size,price,description) values (%s,%s,%s,%s,%s)',
                       ('Ciaz', name, size, data1, description))
        cursor.execute('select tsize_id,tyrename.t_id from tyresize,tyrename where size=%s and name=%s', (size, name))
        data = cursor.fetchone()
        print(data)
...
def batterybalenoupdate(batteryname1):
    if request.method == 'POST':
        ...
        description = request.form['description']
        cursor = mysql.connection.cursor()
        cursor.execute('select capacity,warranty,price from batterytype where model_id=%s and batteryname=%s',
                       (1, batteryname1))
        data = cursor.fetchone()
        cursor.execute(
            'insert into batteryupdate (carname,batteryname,capacity,warranty,price,updateprice,description) values (%s,%s,%s,%s,%s,%s,%s)',
            ('Baleno', batteryname1, data[0], data[1], data[2], price, description))
        cursor.fetchone()
...
def batteryswiftupdate(batteryname1):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into batteryupdate (carname,batteryname,capacity,warranty,price,updateprice,description) values (%s,%s,%s,%s,%s,%s,%s)',
            ('Swift', batteryname1, data[0], data[1], data[2], price, description))
        cursor.fetchone()
...
def batteryspressoupdate(batteryname1):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into batteryupdate (carname,batteryname,capacity,warranty,price,updateprice,description) values (%s,%s,%s,%s,%s,%s,%s)',
            ('Spresso', batteryname1, data[0], data[1], data[2], price, description))
        cursor.fetchone()
...
def batterydzireupdate(batteryname1):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into batteryupdate (carname,batteryname,capacity,warranty,price,updateprice,description) values (%s,%s,%s,%s,%s,%s,%s)',
            ('Dzire', batteryname1, data[0], data[1], data[2], price, description))
        cursor.fetchone()
...
def batteryciazupdate(batteryname1):
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into batteryupdate (carname,batteryname,capacity,warranty,price,updateprice,description) values (%s,%s,%s,%s,%s,%s,%s)',
            ('Ciaz', batteryname1, data[0], data[1], data[2], price, description))
        cursor.fetchone()
...
def ciazbattery():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            "select batteryname from batterytype where model_id=2 and batteryname=%s and capacity=%s and warranty=%s and price=%s",
            (batteryname, capacity, warranty, price))
        data = cursor.fetchall()
        print(data)
        if data:
...
def balenobattery():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            "select batteryname from batterytype where model_id=1 and batteryname=%s and capacity=%s and warranty=%s and price=%s",
            (batteryname, capacity, warranty, price))
        data = cursor.fetchall()
        print(data)
        if data:
...
def dzirebattery():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            "select batteryname from batterytype where model_id=3 and batteryname=%s and capacity=%s and warranty=%s and price=%s ",
            (batteryname, capacity, warranty, price))
        data = cursor.fetchall()
        print(data)
        if data:
...
def spresssobattery():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            "select batteryname from batterytype where model_id=4 and batteryname=%s and capacity=%s and warranty=%s and price=%s ",
            (batteryname, capacity, warranty, price))
        data = cursor.fetchall()
        print(data)
        if data:
...
def swiftbattery():
    if request.method == 'POST':
        ...
        cursor = mysql.connection.cursor()
        cursor.execute(
            "select batteryname from batterytype where model_id=5 and batteryname=%s and capacity=%s and warranty=%s and price=%s",
            (batteryname, capacity, warranty, price))
        data = cursor.fetchall()
        print(data)
        if data:
...
def adddeletedbattery(id, model):
    ...
    if request.method == 'POST':
        ...
        data = cursor.fetchone()
        cursor.execute(
            "select batteryname from batterytype where model_id=5 and batteryname=%s and capacity=%s and price=%s",
            (batteryname, capacity, price,))
        data2 = cursor.fetchall()
        print(data2)
        if data2:
            flash('Service already exists')
            ...
            cursor.execute(
                "INSERT INTO batterytype (batteryname,capacity,warranty,price,model_id) VALUES (%s,%s,%s,%s,%s)",
                (batteryname, capacity, warranty, price, data,))
            mysql.connection.commit()
...
def deletebalenobattery(batteryname1):
    ...
    if request.method == 'POST':
        ...
        print(data)
        cursor.execute(
            'insert into deletebattery (carname,batteryname,capacity,warranty,price,description) values (%s,%s,%s,%s,%s,%s)',
            ('Baleno', batteryname1, data[0], data[1], data[2], description))
...
def deleteswiftbattery(batteryname1):
    ...
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into deletebattery (carname,batteryname,capacity,warranty,price,description) values (%s,%s,%s,%s,%s,%s)',
            ('Swift', batteryname1, data[0], data[1], data[2], description))
...
def deletespressobattery(batteryname1):
    ...
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into deletebattery (carname,batteryname,capacity,warranty,price,description) values (%s,%s,%s,%s,%s,%s)',
            ('Spresso', batteryname1, data[0], data[1], data[2], description))
...
def deletedzirebattery(batteryname1):
    ...
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into deletebattery (carname,batteryname,capacity,warranty,price,description) values (%s,%s,%s,%s,%s,%s)',
            ('Dzire', batteryname1, data[0], data[1], data[2], description))
...
def deleteciazbattery(batteryname1):
    ...
    if request.method == 'POST':
        ...
        cursor.execute(
            'insert into deletebattery (carname,batteryname,capacity,warranty,price,description) values (%s,%s,%s,%s,%s,%s)',
            ('Ciaz', batteryname1, data[0], data[1], data[2], description))
...
def adddeletedrepair(id, model):
    ...
    if request.method == 'POST':
        ...
        cursor.execute("select model_id from model where carname=%s", (model,))
        data = cursor.fetchone()
        cursor.execute("select servicename from repairtype where model_id=%s and servicename=%s and price=%s ",
        ...
        if data2:
            ...
            cursor.execute("INSERT INTO repairtype (servicename,price,Service_Schedule,model_id) VALUES (%s,%s,%s,%s)",
                           (servicename, price, period, data,))
            mysql.connection.commit()
...
def balenorepair():
    if request.method == 'POST':
        ...
        period = request.form['period']
        cursor = mysql.connection.cursor()
        cursor.execute("select servicename from repairtype where model_id=1 and servicename=%s and price=%s ",
                       (servicename, price))
        data = cursor.fetchall()
        print(data)
        if data:
...
def ciazrepair():
    if request.method == 'POST':
        ...
        period = request.form['period']
        cursor = mysql.connection.cursor()
        cursor.execute("select servicename from repairtype where model_id=2 and servicename=%s and price=%s ",
                       (servicename, price))
        data = cursor.fetchall()
        print(data)
        if data:
...
def dzirerepair():
    if request.method == 'POST':
        ...
        period = request.form['period']
        cursor = mysql.connection.cursor()
        cursor.execute("select servicename from repairtype where model_id=3 and servicename=%s and price=%s ",
                       (servicename, price))
        data = cursor.fetchall()
        print(data)
        if data:
...
def spressorepair():
    if request.method == 'POST':
        ...
        period = request.form['period']
        cursor = mysql.connection.cursor()
        cursor.execute("select servicename from repairtype where model_id=4 and servicename=%s and price=%s ",
                       (servicename, price))
        data = cursor.fetchall()
        print(data)
        if data:
...
def swiftrepair():
    if request.method == 'POST':
        ...
        period = request.form['period']
        cursor = mysql.connection.cursor()
        cursor.execute("select servicename from repairtype where model_id=5 and servicename=%s and price=%s",
                       (servicename, price))
        data = cursor.fetchall()
        print(data)
        if data:
        ...
        for value in model:
            ...
            if data1:
                ...
                if data3:
                    ...
                    cursor.execute("insert into tyresize (size,price,t_id) values (%s,%s,%s)",
                                   (tyresize1, price, data2,))
                    cursor.execute("select tsize_id from tyresize where t_id=%s and size=%s", (data2, tyresize1))
                    data8 = cursor.fetchone()
                    print(data8)
                    print(data7)
...
def deletebalenoerepair(servicename1):
    ...
    if request.method == 'POST':
        description = request.form['description']
        cursor = mysql.connection.cursor()
        cursor.execute('select price,Service_Schedule from repairtype where model_id=%s and servicename=%s',
                       (1, servicename1))
        data = cursor.fetchone()
        print(data)
        cursor.execute(
            'insert into deleterepair (carname,servicename,price,service_schedule,description) values (%s,%s,%s,%s,%s)',
            ('Baleno', servicename1, data[0], data[1], description))
...
def enquiry():
    ...
    if 'loggedin' in session:
        if request.method == 'POST' and 'description' in request.form:
            description = request.form['description']
            cursor = mysql.connection.cursor()
            cursor.execute("insert into project.enquiry (description,customer_id) values (%s,%s)",
...
def enquiry_detail():
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select enquiryid,fullname,enquirydate,email_id from enquiry,customer where enquiry.customer_id=customer.customer_id")
    data = cursor.fetchall()
    cursor.close()
...
def respondenquiry(sno):
    cursor = mysql.connection.cursor()
    cursor.execute(
        "select enquiryid,fullname,email_id,description from enquiry,customer where enquiry.customer_id=customer.customer_id and enquiryid=%s",
        (sno,))
    data = cursor.fetchall()
    cursor.close()
    print(data[0])
...
def enquiryresponded(data1, data2):
    if request.method == 'POST':
        ...
        print(description)
        cursor.execute(
            "select description from enquiry,customer where enquiry.customer_id=customer.customer_id and enquiryid=%s",
            (data1,))
        data = cursor.fetchone()
        cursor.execute("insert into project.enquiryresponded (email_id,c_description,adminrespond) values (%s,%s,%s)",
...
def serviceform():
    ...
    print(data4)
    cursor.execute(
        "select batteryname,capacity,warranty,price from batterytype,model where batterytype.model_id=model.model_id and carname=%s",
        ((session['model'],)))
    data5 = cursor.fetchall()
    for value in data5:
        print(value[0])
    ...
    if 'id' in session:
        ...
        if request.method == 'POST':
            ...
            if len(date.today().strftime('%j')) == 2:
                s_id = today_year + '0' + date.today().strftime('%j') + "".join(str(datetime.now().time()).split(':'))[
                                                                        :4]
            elif len(date.today().strftime('%j')) == 1:
                s_id = str(today_year) + '00' + date.today().strftime('%j') + "".join(
                    str(datetime.now().time()).split(':'))[:4]
            elif len(date.today().strftime('%j')) == 3:
                s_id = str(today_year) + date.today().strftime('%j') + "".join(str(datetime.now().time()).split(':'))[
                                                                       :4]
            if deliverytype == "pickup":
                cursor.execute(
                    "INSERT INTO serviceform (s_id,name,model,cartype,r_no,service_type,date,time,deliverytype,pickup,customer_id,pincode) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);",
                    (s_id, cus_id, data4, cartype, registration_no, servicetype, date1, time, deliverytype, pickup,
                     session['id'], pincode,))
                print('hi')
                for value in servicename:
                    ...
                    session['s_id'] = s_id
                    print(session['s_id'])
                    cursor.execute("select servicename,price from repairtype where servicename=%s and model_id=%s",
                                   (value, data7,))
                    data8 = cursor.fetchall()
                    print(data8)
                    for value in data8:
                        cursor.execute("insert into service (s_id,servicename,price) values (%s,%s,%s);",
                                       (s_id, value[0], value[1],))
                        mysql.connection.commit()
                    if value == "ecowashing":
                ...
                cursor.execute(
                    "INSERT INTO serviceform (s_id,name,model,cartype,r_no,service_type,deliverytype,dropaddress,customer_id,pincode) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);",
                    (s_id, cus_id, data4, cartype, registration_no, servicetype, deliverytype, dropaddress,
                     session['id'], pincode,))
                for value in servicename:
                    # cursor.execute("insert into service (s_id,servicename) values (%s,%s);",(s_id,value,))
                    session['s_id'] = s_id
                ...
                cursor.execute(
                    "INSERT INTO serviceform (s_id,name,model,cartype,r_no,service_type,date,time,deliverytype,pickup,dropaddress,customer_id,pincode) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);",
                    (s_id, cus_id, data4, cartype, registration_no, servicetype, date2, time2, deliverytype, pickup1,
                     dropaddress1, session['id'], pincode,))
                for value in servicename:
                    # cursor.execute("insert into service (s_id,servicename) values (%s,%s);",(s_id,value,))
                    cursor.execute("select servicename,price from repairtype where servicename=%s and model_id=%s",
                                   (value, data7,))
                ...
                cursor.execute(
                    "INSERT INTO serviceform (s_id,name,model,cartype,r_no,service_type,customer_id,pincode) VALUES (%s,%s,%s,%s,%s,%s,%s,%s);",
                    (s_id, cus_id, data4, cartype, registration_no, servicetype, session['id'], pincode,))
                print('hi')
                for value in servicename:
                    print(value)
                    # cursor.execute("insert into service (s_id,servicename) values (%s,%s);",(s_id,value,))
                ...
                if "tyrechange" in servicename and "batterychange" in servicename:
                    ...
                    # mail.send(msg)
                    return redirect(url_for('userdashboard'))
            return render_template('serviceform.html', msg=msg, data=data, data1=data1, data3=data3, data4=data4,
                                   data5=data5, data6=data6, data11=data11)
        else:
            msg = 'please fill up the form!'
            return render_template('serviceform.html', msg=msg, data=data, data1=data1, data3=data3, name=name,
                                   data4=data4, data5=data5, data6=data6, data11=data11)
...
def tyrename():
    ...
    if request.method == 'POST':
        ...
        print(tname)
        cursor.execute(
            "select price from tyresize,model,tyremodel,tyrename where tyrename.t_id=tyresize.t_id and tyresize.tsize_id=tyremodel.tsize_id and model.model_id=tyremodel.model_id and carname=%s and name=%s and size=%s",
            (session['model'], tname, size,))
        price = cursor.fetchone()
        cursor.execute("insert into servicetyre (s_id,tyrename,size,price,tno) values (%s,%s,%s,%s,%s)",
                       (session['s_id'], tname, size, price, no,))
        mysql.connection.commit()
        cursor.execute("update service set price=%s where s_id=%s and servicename=%s",
                       (price, session['s_id'], "tyrechange",))
        mysql.connection.commit()
...
def tyrebattery():
    ...
    for value in data:
        print(value[0])
    cursor.execute(
        "select batteryname,capacity,warranty from batterytype,model where batterytype.model_id=model.model_id and carname=%s",
        ((session['model'],)))
    data1 = cursor.fetchall()
    for value in data1:
        print(value[0])
    ...
    if request.method == 'POST':
        ...
        print(session['model'])
        cursor.execute(
            'select price from batterytype,model where batterytype.model_id=model.model_id and carname=%s and batteryname=%s and capacity=%s',
            (session['model'], bname, capacity,))
        data2 = cursor.fetchone()
        print(data2)
        print(bname)
        print(capacity)
        ...
        mysql.connection.commit()
        for value in data2:
            cursor.execute("insert into servicebattery (s_id,batteryname,price,capacity) values (%s,%s,%s,%s)",
                           (session['s_id'], bname, value, capacity,))
            mysql.connection.commit()
            cursor.execute("update service set price=%s where s_id=%s and servicename=%s",
                           (data2, session['s_id'], "batterychange",))
            mysql.connection.commit()
        cursor.execute("select servicename from service where s_id=%s", (session['s_id'],))
        data9 = cursor.fetchall()
        print(data9)
